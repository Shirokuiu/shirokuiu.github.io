...
(function () {
  ...
  if (hero !== null) {
    ...
    (function () {
      var clockData1 = hero.querySelector('.timer1');

      var dt = clockData1.dataset.timeRemaining;
      ...
      var clock1 = $('.hero .timer1').FlipClock(remaining, {
    ...
    (function () {
      var clockData2 = hero.querySelector('.timer2');

      var dt = clockData2.dataset.timeRemaining;
      var first = new Date(dt);
      var last = Date.now();
      var remaining = first - last;
      ...
      var clock2 = $('.hero .timer2').FlipClock(remaining, {
        clockFace: 'DailyCounter',
        countdown: true,
        language: 'ru-ru'
      });
    ...
    (function () {
      var clockData3 = hero.querySelector('.timer3');

      var dt = clockData3.dataset.timeRemaining;
      var first = new Date(dt);
      var last = Date.now();
      var remaining = first - last;
      ...
      var clock2 = $('.hero .timer3').FlipClock(remaining, {
        clockFace: 'DailyCounter',
        countdown: true,
        language: 'ru-ru'
      });
...
(function () {
  var goodInner = document.querySelector('.good-inner');

  if (goodInner !== null) {
    var calc = goodInner.querySelector('.calc');
    var priceWrap = goodInner.querySelector('.row .desc .price-wrap .price');
    var price = priceWrap.querySelector('span');

    price.textContent = priceWrap.dataset.price;
    ...
    calc.addEventListener('click', function (evt) {
      ...
      if (minus) {
        if (parseInt(val.textContent, 10) > 1) {
          ...
          price.textContent = window.addSpace(parseInt(window.removeSpace(price.textContent), 10) - parseInt(window.removeSpace(priceWrap.dataset.price), 10));
        };
      };
...
(function () {
  ...
  if (goods !== null) {
    var sort = goods.querySelector('.goods .sort');
    ...
    if (sort !== null) {
      ...
      select.addEventListener('click', function (evt) {
        ...
        var lineText = this.querySelector('.line .text');

        this.classList.toggle('open');

        if (items) {
          lineText.textContent = items.textContent;
        };
      });
    };
  };
...
(function () {
  ...
  if (order !== null) {
    (function () {
      var table = order.querySelector('table');
      var tableItems = table.querySelectorAll('tbody tr');
      var totalPrice = order.querySelector('form .row .left .total-price > span');
      var tableItemsPri = order.querySelectorAll('form table tbody td:nth-child(3) span');

      var calcTotalPrice = function calcTotalPrice(elem) {
        var j = 0;

        for (var i = 0, len = elem.length; i < len; i++) {
          j = j + parseInt(window.removeSpace(elem[i].textContent), 10);
        };
      ...
      for (var i = 0, len = tableItems.length; i < len; i++) {
        var tableItemsPrice = tableItems[i].querySelector('form table tbody td:nth-child(3) span');
        var tableItemsVal = tableItems[i].querySelector('form table tbody td .calc span');
        var calc = tableItems[i].querySelector('.calc');

        tableItemsPrice.textContent = window.addSpace(parseInt(window.removeSpace(tableItems[i].dataset.price), 10) * parseInt(tableItemsVal.textContent, 10));
        ...
        calc.addEventListener('click', function (evt) {
          ...
          var val = this.querySelector('span');
          var plus = evt.target.closest('.plus');
          var thisPrice = par.querySelector('td:nth-child(3) span');

          if (minus) {
            if (parseInt(val.textContent, 10) > 1) {
              val.textContent = parseInt(val.textContent, 10) - 1;
              thisPrice.textContent = window.addSpace(parseInt(window.removeSpace(thisPrice.textContent), 10) - parseInt(window.removeSpace(par.dataset.price), 10));
              totalPrice.textContent = window.addSpace(calcTotalPrice(tableItemsPri));
            };
          };
...
(function () {
  ...
  var basket = nav.querySelector('.basket');
  ...
  var navPos = getCoords(pageContent);
...
(function () {
  var btn = document.querySelector('.page-header button');
  var nav = document.querySelector('.nav');
