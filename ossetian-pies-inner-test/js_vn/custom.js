...
window.onload = function() {
	...
	$(document).ready(function() {
	    ...
	    $('form.form-review').submit(function(e){
			e.preventDefault();
			var $form = $(this);
			$form.find('.form-review__submit').prop('disabled','disabled');
	      	$.ajax({
	       		url: '/review/',
	        	type: 'POST',
				dataType: 'json',
	        	data: {
	          		user_name: $form.find("input.form-review__input--name").val(),
	          		user_date: $form.find("input.form-review__input--date").val(),
	          		user_review: $form.find("textarea.form-review__textarea").val()
	        	},
	        	error: function() {
	          		$form.find('.form-review__submit').prop('disabled',false);
	        	},
	        	success: function(resp) {
	       			if(!resp.errors) {
		            	...
		            	swal({
	  					...
	  					setTimeout(function() {$form[0].reset();$form.find('.form-review__submit').prop('disabled',false);}, 2000);
	  				}
	        	}
	      	}); 
		});
