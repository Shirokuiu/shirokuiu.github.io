...
;(function($) {
  ...
  var actions = {
    ...
    showProcess: function (item){
      ...
      $('body').append('<div class="jqcart-layout-info">'+
        '<div class="jqcart-content-info">'+
            item.title+' в количестве '+item.cnt+' шт <br>добавлен в корзину'+
        '</div>'+
      '</div>');
      $('.jqcart-layout-info').fadeIn(200);
      setTimeout(function(){$('.jqcart-layout-info').fadeOut(600);$('.jqcart-layout-info').remove();}, 2000);
    },
    hideCart: function() {
    ...
    sendOrder: function(e) {
      ...
      if ((inputname.val()=="") && valid) { 
        ...
        setTimeout(function(){inputname.val(''); inputname.focus(); inputname.attr("style","border: 1px #C0C0C0 solid;-moz-border-radius: 3px;-webkit-border-radius: 3px;border-radius: 3px;");}, 1000);
      }     
      ...
      if ((inputphone.val()=="") && valid){
        ...
        setTimeout(function(){inputphone.val(''); inputphone.focus(); inputphone.attr("style","border: 1px #C0C0C0 solid;-moz-border-radius: 3px;-webkit-border-radius: 3px;border-radius: 3px;");}, 1000);
      }
      ...
      $.ajax({
        ...
        data: {
          ...
          address: $form.find("input[name=user_address]").val(),
          comment: $form.find("textarea[name=user_comment]").val()
        },
        error: function() {},
  ...
  var methods = {
		clearCart: function(){
			DB.removeItem('jqcart_r165783987');
