...
(function () {
  var afishaTable = document.querySelector('.afisha__table');

  if (afishaTable !== null) {
    var afishaItems = afishaTable.querySelectorAll('.afisha__table-items');

    for (var i = 0, len = afishaItems.length; i < len; i++) {
      afishaItems[i].addEventListener('click', function (evt) {
        var btn = evt.target.closest('.afisha__line .td:nth-child(6) button');
        var subMenu = this.querySelector('.afisha__sub-menu');

        if (btn) {
          var btnText = btn.querySelector('.wrap .text');

          this.classList.toggle('active');
          btn.classList.toggle('active');
          subMenu.classList.toggle('afisha__sub-menu--close');

          if (this.classList.contains('active')) {
            btnText.textContent = 'cвернуть';
          } else {
            btnText.textContent = 'Выбрать билеты';
          };
        };
      });
    };
  ...
  ScrollReveal().reveal('.afisha__table-items', {
    delay: 50,
    scale: 0.9,
    interval: 50,
    easing: 'ease',
...
(function () {
  var basketTicket = document.querySelector('.basket-ticket');

  if (basketTicket !== null) {
    ScrollReveal().reveal('.basket__table tbody tr', {
      delay: 50,
      scale: 0.9,
      interval: 50,
      easing: 'ease',
...
(function () {
  var icon = document.querySelectorAll('.schema__places a');
  var iconPopup = document.querySelector('.notice');
  var iconPopupCont = iconPopup.querySelector('.cont');
  ...
  for (var i = 0, len = icon.length; i < len; i++) {
    icon[i].addEventListener('mouseenter', function (evt) {
      var popupHeight = iconPopup.getBoundingClientRect().height;
      var popupWidth = iconPopup.getBoundingClientRect().width;

      iconPopup.classList.add('hover');
      iconPopup.style.left = getCoords(this).left - popupWidth / 2 + 9 + 'px';
      iconPopup.style.top = getCoords(this).top - popupHeight - 20 + 'px';
      clearTimeout(hidePopup);

      switch (this.classList[0]) {
        case 'no_tickets_1':
          iconPopupCont.textContent = 'Продано';
          break;
        case 'mesto_1':
          iconPopupCont.innerHTML = this.getAttribute('title', '');
          break;
        case 'in_basket_1':
          iconPopupCont.textContent = 'Билет в корзине. Хотите удалить?';
      };
    });

    icon[i].addEventListener('click', function (evt) {
      evt.preventDefault();
    });
  };

  for (var i = 0, len = icon.length; i < len; i++) {
    icon[i].addEventListener('mouseleave', function (evt) {
      hidePopup = setTimeout(function () {
        iconPopup.classList.remove('hover');
      }, 300);
    });
  };
})();
...
(function () {
  ...
  var menu = pageHeader.querySelector('.page-header ul');
  var btn = pageHeader.querySelector('.page-header button');
